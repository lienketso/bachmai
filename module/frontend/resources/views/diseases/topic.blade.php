    ...
    <section class="diseas_page pdt50 pdb50">
        <div class="container">
            ...
            <div class="row">
                ...
                <div class="col-lg-6">
                    <h3 class="title_ap">{{trans('frontend.all_topic')}}</h3>
                    <div class="list_desi">
                        @if(!empty($data))
                            @foreach($data as $d)
                                <div class="desi_item">
                                    <a href="{{route('frontend::diseases.index.get',['topic'=>$d->slug])}}">{{$d->name}}</a>
                                        @if(!empty($d->post) && count($d->post) > 0)
                                        <ul class="post_sub">
                                            @foreach($d->post as $p)
                                                <li><a href="{{route('frontend::diseases.detail.get',$p->slug)}}">{{$p->name}}</a></li>
                                            @endforeach
                                        </ul>
                                        @endif
